...
export default function Question({ ques, ans, important, selectedAns, onSelectAns, onNextQues, onPrevQues }: QuestionProps) {
    return (
        <div className="relative flex justify-center  h-screen overflow-hidden bg-linear-to-l from-[#DAE82C] via-[#3C8247] via-[#0A483D] to-[#092B25]">
            <div className="max-w-7xl space-y-8 mt-40">
                ...
                <div className="flex justify-start gap-10">
                    {ans.map((eachAns, i) => {
                        ...
                        return (
                            <div key={i} className={`flex flex-col cursor-pointer p-4 hover:ring-1 rounded-3xl ${isSelected ? 'ring-4 ring-white' : ''}`} onClick={() => onSelectAns(eachAns.ans)}>
                                <div className=''>
                                    {eachAns.imgSrc &&
